...
void LCD_init(void)
{
      ...
      RS=0;                   // set low to write control data (to instruction register)
      E=0;                    // set low

      //function mode
      data=0x2;               // 0010, 4 bit mode (data packet 1, DB4-DB7)
      toggle_enable();
      data=0x8;               // 1000, 2-line, 7 dot char (data packet 2, DB0-DB3)
      toggle_enable();
      //display mode
      data=0x0;               // 0000, 4 bit mode (data packet 1, DB4-DB7)
      toggle_enable();
      data=0xF;               // 1111, display on, cursor on, blink on
      toggle_enable();

      //clear display
      data=0x0;
      toggle_enable();
      data=0x1;               // 0000 0001-->0x01, clear 
      toggle_enable();
}
...
{
      RS=1;               // (data register) set high to write character data
